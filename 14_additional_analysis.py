...
# # Define the pixel bounds of CONUS on the image

# # Adjust the stride based on the total number of models
# model_index = 0
# one_model_results = r_squared_array[model_index::6]  

# # Initialize an array to hold the cropped R-squared values, fill with NaN for missing data
...
#         # Adjust the row and col indices based on the cropped bounds
#         cropped_row_index = row - upper_bound
#         cropped_col_index = col - left_bound
#         cropped_r_squared_values[cropped_row_index, cropped_col_index] = r_squared


# In[24]:


# # Define the gridline frequency for major and minor ticks
# major_interval = 50  # Major grid line every 50 pixels
# minor_interval = 10  # Minor grid line every 10 pixels within major intervals

# # Create the plot
# fig, ax = plt.subplots(figsize=(15, 10))  # Adjust the figure size as needed
# im = ax.imshow(cropped_r_squared_values, cmap='Reds', interpolation='none')  # 'none' prevents smoothing NaNs
# fig.colorbar(im, ax=ax, label='R-squared values')  # Add a colorbar for reference
# ax.set_title('R-squared values for one model')
...
# ax.set_xticks(np.arange(0, cropped_r_squared_values.shape[1], minor_interval), minor=True)
# ax.set_yticks(np.arange(0, cropped_r_squared_values.shape[0], minor_interval), minor=True)
# # ax.grid(which='minor', color='gray', linestyle=':', linewidth=0.5)

# # Customize the tick labels to show whole numbers
# # This assumes the first tick is at 0 and increases by the major interval
# ax.set_xticklabels(range(0, cropped_r_squared_values.shape[1], major_interval))
# ax.set_yticklabels(range(0, cropped_r_squared_values.shape[0], major_interval))
...
#     one_model_results = r_squared_array[model_index::num_models]  # Extract results for one model

#     # Initialize an array for the cropped R-squared values
#     cropped_r_squared_values = np.full((lower_bound - upper_bound, right_bound - left_bound), np.nan)
...
#             cropped_row_index = row - upper_bound
#             cropped_col_index = col - left_bound
#             cropped_r_squared_values[cropped_row_index, cropped_col_index] = max(r_squared, 0.1)  # Set values less than 0.1 to 0

#     # Plot the data
#     plt.figure(figsize=(15, 10))
#     plt.imshow(cropped_r_squared_values, cmap='Reds', interpolation='none')
#     plt.colorbar(label='R-squared values')
#     plt.title(f'R-squared values for model {model_index}ML')
#     plt.savefig(f'cropped_r_squared_{model_index}ML.png')  # Save the plot
#     plt.show()

#     # Save the cropped R-squared array
#     np.save(f'cropped_r_squared_{model_index}ML.npy', cropped_r_squared_values)
