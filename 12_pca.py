...
# Assuming 'SPEI' is the column with SPEI values in merged_df
...
features = ['SPEI_lag_0', 'SPEI_lag_1', 'SPEI_lag_2', 'SPEI_lag_3', 'SPEI_lag_4', 'SPEI_lag_5']
x = merged_df[features].values
y = merged_df['Search Interest'].values
...
print(pca.explained_variance_ratio_)  # Explained variance ratio of each component

# Combine PCA results with the original data for more in-depth analysis
merged_pca_df = pd.concat([pca_df, merged_df[['Search Interest']]], axis=1)


# In[17]:


# Analyze correlation between PCA components and search interest
correlation_matrix = merged_pca_df.corr()
...
pca_results['Month'] = pd.to_datetime(pca_results['Month'])

# Group by Month and calculate mean PC1 score for each month
monthly_avg_pca = pca_results.groupby('Month')['PC1'].mean().reset_index()

# Plotting
plt.figure(figsize=(10, 5))
plt.plot(monthly_avg_pca['Month'], monthly_avg_pca['PC1'], marker='o', linestyle='-', color='blue')
...
# State abbreviations (you will need to create this mapping or use a reliable source)
state_abbreviations = {
...
# Create a new dictionary that maps state abbreviations to regions
regions_abbreviations = {state_abbreviations[state]: region for region, states in regions.items() for state in states}

# Now you can map the 'State' column in pca_aggregated which has state abbreviations
pca_aggregated['Region'] = pca_aggregated['State'].map(regions_abbreviations)

# Aggregate PCA scores by region
region_pca_scores = pca_aggregated.groupby('Region')['PC1'].mean().reset_index()

# Map state abbreviations to regions
gdf_states['Region'] = gdf_states['NAME'].map(state_to_region)

# Now you can dissolve gdf_states by 'Region' to get regional geometries
